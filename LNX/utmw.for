c    ---------------------------------------------------------  U T M W
c...  This program transforms UTM coordinates to MTMw    
c...  over a UTM zone, extending fron Latitude 42 to 75
      ...
      DATA U/(4653917.463d0,251528.228d0),(6488072.911d0,325180.594d0),
     ...
     +       (6487091.342d0,587449.541d0),(8326498.728d0,543346.368d0)/
      ...
      OPEN(1,file='uTw')                          ! uTw grid coords
      OPEN(2,file='mTw')                          ! mTw grid coords
      OPEN(6,file='mtmW.lst')                     ! List file
      WRITE(6,15)
      WRITE(6,25) 
 
 10   CONTINUE
      READ(1,*,end=50) K , Z                      ! Get uTw coords
      READ(2,*,end=50) K , W                      ! Get mTw coords    w
      Z0 = Z                                      ! Get a data point.
      ZX = Z0                                     ! If it is from UTM
      IF(AIMAG(ZX).GT.500000.) GOTO 10            ! E-half - Skip it!
      FZ = (0.0,0.0)                           
 ...
 50   STOP ' <* UT-MW Completed :> mtmW.lst *>'
 15   FORMAT(79('='),/,'* TRANSFORM:',6x,'UTMw',11x,'TO',13x,'MTMw',
     +       14x,'(Grid-Calc) *')
