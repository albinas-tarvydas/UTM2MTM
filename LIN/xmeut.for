c    ---------------------------------------------------------  X M E U T
c...  This routine transforms the selected info in MTMe to UTM

      BLOCK DATA 
      IMPLICIT     DOUBLE COMPLEX (U-V)  
      COMMON/ARGS/ U(6) , V(6)           
      DATA U/(4652046.692d0,624268.651d0),(6487091.342d0,587449.541d0),
     +       (8326498.728d0,543346.368d0),(4652046.692d0,375731.349d0),
     +       (6487091.342d0,412550.459d0),(8326498.728d0,456653.632d0)/
      DATA V/(4653917.463d0,748471.772d0),(6488072.911d0,674819.406d0),
     ...
     +       (6484169.199d0,500000.000d0),(8323452.589d0,500000.000d0)/
      ...
      OPEN(1,file='mTe')                          ! MTMe grid coords
      OPEN(2,file='uTe')                          ! UTMe grid coords
      OPEN(6,file='xutmE.lst')                    ! List file 
      WRITE(6,15)

 10   CONTINUE
      READ(1,*,end=50) CD , Z                     ! Get MTMe coords
      READ(2,*,end=50) CD , W                     ! Get UTMe coords
 ...
 50   STOP ' <* XME-UT Completed :> xutmE.lst *>'
 15   FORMAT(80('='),/,'*',19x,'*',6x,'UTMe DATA',6x,'*',8x,
     +       '(  TRANSFORMED FROM  MTMe   )',/,80('_'),/,25x,'Y',10x,
